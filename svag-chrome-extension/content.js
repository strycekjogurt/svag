...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // ===== BULK DOWNLOAD MESSAGES =====
  if (request.action === 'detectAllSvgs') {
    console.log('[svag v1.2] Received detectAllSvgs request from popup');
    try {
      const allSvgs = detectAllSvgsOnPage();
      sendResponse({ success: true, count: allSvgs.length });
    } catch (error) {
      console.error('[svag v1.2] Error detecting SVGs:', error);
      sendResponse({ success: false, error: error.message, count: 0 });
    }
    return true; // Pro async response
  }
  
  if (request.action === 'downloadAllSvgs') {
    console.log('[svag v1.2] Received downloadAllSvgs request from popup');
    
    // Spustit download asynchronně
    downloadAllSvgs().then((result) => {
      sendResponse(result);
    }).catch((error) => {
      console.error('[svag v1.2] Download error:', error);
...
/**
 * Detekuje všechny SVG elementy na stránce
 * @returns {Array} Pole objektů s SVG daty
 */
function detectAllSvgsOnPage() {
  console.log('[svag v1.2] Detecting all SVGs on page...');
  
  const allSvgs = [];
  const seenContents = new Set(); // Pro filtrování duplikátů
  
  // 1. Skenovat inline SVG elementy
  const inlineSvgs = document.querySelectorAll('svg');
  console.log(`[svag v1.2] Found ${inlineSvgs.length} inline SVG elements`);
  
  inlineSvgs.forEach((svg) => {
    if (isSvgElement(svg)) {
      const svgData = getSvgData(svg);
      if (svgData) {
        // Filtrovat duplikáty podle obsahu
        const contentHash = svgData.content ? svgData.content.substring(0, 100) : '';
  ...
  // 2. Skenovat IMG elementy s SVG
  const imgSvgs = document.querySelectorAll('img[src*=".svg"], img[src^="data:image/svg"]');
  console.log(`[svag v1.2] Found ${imgSvgs.length} IMG elements with SVG`);
  
  imgSvgs.forEach((img) => {
    const svgData = getSvgData(img);
    if (svgData) {
      const contentHash = svgData.url || svgData.content?.substring(0, 100) || '';
      if (!seenContents.has(contentHash)) {
        seenContents.add(contentHash);
        allSvgs.push(svgData);
      }
  ...
  // 3. Skenovat shadow DOM
  const shadowRootSvgs = scanShadowRoots(document.body);
  console.log(`[svag v1.2] Found ${shadowRootSvgs.length} SVG elements in shadow DOM`);
  
  shadowRootSvgs.forEach((svg) => {
    const svgData = getSvgData(svg);
    if (svgData) {
      const contentHash = svgData.content ? svgData.content.substring(0, 100) : '';
      if (!seenContents.has(contentHash)) {
  ...
  console.log(`[svag v1.2] Total unique SVGs detected: ${allSvgs.length}`);
  return allSvgs;
}

/**
 * Stáhne všechny SVG na stránce jako ZIP
 */
async function downloadAllSvgs() {
  console.log('[svag v1.2] Starting bulk download...');
  
  try {
    // Detekovat všechny SVG
    const allSvgs = detectAllSvgsOnPage();
    
    if (allSvgs.length === 0) {
      showNotification('no icons found', null);
    ...
    // Vytvořit ZIP pomocí JSZip
    const zip = new JSZip();
    let successCount = 0;
    const usedNames = new Set(); // Pro unikátní názvy souborů
    
    // Zpracovat každý SVG
    for (let i = 0; i < allSvgs.length; i++) {
      const svgData = allSvgs[i];
      
      try {
        // Získat čistý SVG obsah
        let content = svgData.content;
        let iconName = null;
        
        // Pokud je to inline SVG nebo use-resolved, použít extractCleanSvg
        if (svgData.element && (svgData.type === 'inline' || svgData.type === 'use-resolved')) {
          let svgElement = svgData.element;
          ...
          if (svgElement) {
            ...
            iconName = cleanData.name;
          }
        }
        
        // Pro img/url typy načíst obsah
        if (!content && svgData.url) {
          try {
            const response = await fetch(svgData.url);
            content = await response.text();
          } catch (error) {
            console.warn(`[svag v1.2] Failed to fetch SVG from URL: ${svgData.url}`, error);
            continue;
          }
        }
        
        if (!content) {
          console.warn(`[svag v1.2] No content for SVG ${i}`);
          continue;
        }
        ...
        if (totalSize > MAX_SIZE) {
          console.warn('[svag v1.2] ZIP size limit reached');
          break;
        }
        
        // Získat název souboru
        if (!iconName && svgData.element) {
          iconName = extractIconName(svgData.element);
        }
        
        // Fallback: použít číslo
        if (!iconName) {
          iconName = `icon-${i + 1}`;
        }
        
        // Zajistit unikátní název
        let finalName = iconName;
        let counter = 1;
        while (usedNames.has(finalName)) {
          finalName = `${iconName}-${counter}`;
          counter++;
        }
        usedNames.add(finalName);
        
        // Přidat do ZIP
        zip.file(`${finalName}.svg`, content);
        successCount++;
        
      } catch (error) {
        console.warn(`[svag v1.2] Error processing SVG ${i}:`, error);
      }
    }
    
    if (successCount === 0) {
      showNotification('no icons extracted', null);
      return { success: false, error: 'No icons extracted' };
    }
    
    // Generovat ZIP
    console.log(`[svag v1.2] Generating ZIP with ${successCount} icons...`);
    const zipBlob = await zip.generateAsync({ type: 'blob' });
    const zipUrl = URL.createObjectURL(zipBlob);
    const zipSizeKB = (zipBlob.size / 1024).toFixed(1);
    
    // Název ZIP ze stránky
    const hostname = window.location.hostname.replace(/\./g, '-');
    const zipFilename = `${hostname}.zip`;
    
    console.log(`[svag v1.2] ZIP ready: ${zipFilename} (${zipSizeKB}kb)`);
    
    // Stáhnout pomocí background scriptu
    chrome.runtime.sendMessage({
      action: 'download',
      url: zipUrl,
      ...
      if (chrome.runtime.lastError) {
        console.error('[svag v1.2] Runtime error:', chrome.runtime.lastError);
        showNotification('download failed', null);
      } else if (response && response.success) {
        showNotification(`${successCount} icons → ${zipFilename}`, null);
      } else {
        console.error('[svag v1.2] Download failed:', response?.error);
        showNotification('download failed', null);
      }
    });
    
    return { success: true, count: successCount, filename: zipFilename };
    
  } catch (error) {
    console.error('[svag v1.2] Bulk download error:', error);
    showNotification('download error', null);
...
console.log('🆕 v1.2 BULK DOWNLOAD: Hromadné stahování všech ikon do ZIP!');

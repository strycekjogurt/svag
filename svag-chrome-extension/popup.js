    ...
    try {
      ...
      if (error.message.includes('Failed to fetch') || 
        ...
        errorMessage = `Cannot connect to server at ${apiUrl}\n\n` +
          ...
          `3. Try opening: https://www.svag.pro/health`;
...
function attachDownloadAllListeners() {
  console.log('🚀 [v1.2] Attaching Download All listeners');
  
  // Získat element zde, když už je DOM ready
  ...
  // Detekovat ikony při otevření popupu
  detectIconsOnPage(downloadAllBtn);
  ...
  downloadAllBtn.addEventListener('click', async () => {
    ...
    const countSpan = downloadAllBtn.querySelector('.download-count');
    const originalText = countSpan.textContent;
    countSpan.textContent = '...';
    
    try {
      // Poslat zprávu do content scriptu pro stažení všech ikon
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
      
      chrome.tabs.sendMessage(tab.id, { action: 'downloadAllSvgs' }, (response) => {
        ...
        if (response && response.success) {
          console.log(`✅ Download successful: ${response.count} icons`);
          // Zobrazit úspěšnou notifikaci
          countSpan.textContent = '✓';
          
          // Po 2 sekundách resetovat na původní počet
          setTimeout(() => {
            countSpan.textContent = originalText;
            downloadAllBtn.disabled = false;
          }, 2000);
...
async function detectIconsOnPage(downloadAllBtn) {
  console.log('🔍 Detecting icons on page...');
  
  // Kontrola, zda je tlačítko dostupné
  if (!downloadAllBtn) {
    console.warn('⚠️ Download All button not available');
    return;
  }
  ...
  try {
    ...
    // Kontrola, zda je to validní tab (ne chrome:// nebo extension page)
    if (!tab || !tab.url || tab.url.startsWith('chrome://') || tab.url.startsWith('chrome-extension://')) {
      console.log('⚠️ Cannot detect icons on this page type');
      downloadAllBtn.style.display = 'none';
      return;
    }
    
    // Poslat zprávu do content scriptu pro detekci ikon
    chrome.tabs.sendMessage(tab.id, { action: 'detectAllSvgs' }, (response) => {
      if (chrome.runtime.lastError) {
        console.warn('⚠️ Content script not ready or page type not supported:', chrome.runtime.lastError);
        downloadAllBtn.style.display = 'none';
        return;
      }
      ...
      if (response && response.success) {
        ...
        console.log(`✅ Detected ${count} icons on page`);
        
        if (count > 0) {
          // Zobrazit tlačítko s počtem
          const countSpan = downloadAllBtn.querySelector('.download-count');
          countSpan.textContent = count;
          downloadAllBtn.style.display = 'flex';
          downloadAllBtn.disabled = false;
          
          // Aktualizovat tooltip s počtem SVG
          downloadAllBtn.setAttribute('data-tooltip', `Download all ${count} SVGs`);
        } else {
          // Žádné ikony nenalezeny
          console.log('ℹ️ No icons found on page');
          downloadAllBtn.style.display = 'none';
        }
      } else {
        console.error('❌ Error detecting icons:', response?.error);
        downloadAllBtn.style.display = 'none';
      }
    });
  } catch (error) {
...
function initTooltips() {
  ...
  const showTooltip = (element) => {
    // Odstranit předchozí tooltip pokud existuje
    if (currentTooltip) {
      currentTooltip.remove();
      currentTooltip = null;
    }
    
    // Číst tooltip text při každém hover (pro dynamické tooltips)
    ...
    // Pozice nad tlačítkem, centrovaná
    let left = rect.left + (rect.width / 2) - (tooltipRect.width / 2);
    let top = rect.top - tooltipRect.height - 8; // 8px offset nad tlačítkem
    
    // Zajistit, že tooltip nezapadne mimo popup
    if (left < 8) left = 8;
    if (left + tooltipRect.width > window.innerWidth - 8) {
      left = window.innerWidth - tooltipRect.width - 8;
    }
    if (top < 8) {
      // Pokud není místo nahoře, zobrazit dole
      top = rect.bottom + 8;
    }
  ...
  const hideTooltip = () => {
    if (currentTooltip) {
      ...
      }, 200); // Čekat na konec animace
    }
  };
  
  // Delegovaný event listener pro všechny elementy s data-tooltip
